...
// second time these players are not added since the updated list was saved back to
// the file.  However, player Fezzik is removed in this program, so the second time
// it is run note that Fezzik must be added.
...
// Read over this file very carefully and note the methods that you must implement
// and their behaviors.  See comments below for more information.

// For some help with these files you should look at Lab6 and, in particular the
...
public class PlayerListTest
{
	public static void main(String [] args) throws IOException
	{
		...
		System.out.println("Array capacity: " + allPlayers.capacity());
		System.out.println();
		...
		for (String S: names)
		{
			...
			String name = onePlayer.name();  // getName() method return's name
			
			// Now the update() method is part of the Player class.  This will allow
			// a single Player's stats to be updated after each round of the game
			System.out.println("Updating player " + name + ":");
			System.out.println("\tWon a round with 3 guesses...");
			onePlayer.update(true, 3);
			System.out.println("\tLost a round with 5 guesses...");
			onePlayer.update(false, 5);
			System.out.println("Showing " + name + " after updates: ");
			System.out.println(onePlayer.toString());
		}
		...
		// The toStringAdmin() method has all of the info from toString and
		// also adds the individual player information to the
		...
		System.out.println("\nSaving player list back to file...");
		allPlayers.saveList();
	}		
}